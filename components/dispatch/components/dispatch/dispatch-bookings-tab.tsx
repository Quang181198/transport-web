...
export default function DispatchBookingsTab({ onOpenGanttMonth }: Props) {
  ...
  async function saveBooking() {
    ...
    try {
      ...
      await loadBookings()
      await loadBookingDetail(detail.id)
      setIsEditing(false)
  ...
  function openGantt() {
    const month = detail.startDate?.slice(0, 7) || detail.endDate?.slice(0, 7)
    if (!month) {
      alert('Booking chưa có ngày để mở đúng tháng trên Gantt')
      return
    }

    onOpenGanttMonth(month)
  }
  ...
  return (
    ...
    >
      <div className="section-card" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        <div style={{ padding: 16, borderBottom: '1px solid #eef3f8' }}>
          <input
            className="input"
            placeholder="Search code đoàn, tên đoàn, phone, email..."
            value={searchKeyword}
